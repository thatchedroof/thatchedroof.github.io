...
export type Company = [string, string, string, string, number, number, number];

export const companies = companyText.split('\n').map(line => {
    return [...(line.split('	') as [string, string, string, string]), ...([Math.round((Math.random() * 9.9 + .1) * 100) / 100, Math.round(Math.random() * 90000 + 10000)] as [number, number]), 0] as Company;
});

console.log(companies);

export const chooseCompanies = (i: number) => companies.sort(() => .5 - Math.random()).slice(0, i);
...
export const nextPrice = (p: number, interval: number, mean: number, sigma: number) => {
    return p * Math.exp((mean - ((sigma * sigma) / 2)) * interval + sigma * normal());
};

export const nextCompanyPrice = (c: Company, interval: number, mean: number, sigma: number) => {
    return [c[0], c[1], c[2], c[3], nextPrice(c[4], interval, mean, sigma), c[5], c[6]] as Company;
};

// // From https://instil.co/blog/crazy-powerful-typescript-41/
// type BuildTuple<Current extends [...T[]], T, Count extends number> =
//     Current["length"] extends Count
//     ? Current : BuildTuple<[T, ...Current], T, Count>;

// type Tuple<T, Count extends number> = BuildTuple<[], T, Count>;

export type StockHistory = [Company[], number, number[], number[]][];

export const addToStockHistory = (history: StockHistory, interval: number | undefined = undefined, mean: number[] | undefined = undefined, sigma: number[] | undefined = undefined): StockHistory => {
    const last = history[history.length - 1];
    return history.concat(
        [[
            last[0].map((c, i) => nextCompanyPrice(
                c, (interval != null ? interval : last[1]), (mean != null ? mean[i] : last[2][i]), (sigma != null ? sigma[i] : last[3][i])
            )),
            (interval ?? last[1]),
            (mean ?? last[2]),
            (sigma ?? last[3])
...
export const simulateStocks = (history: StockHistory, count: number = 1): StockHistory => {
    if (count <= 0) {
        return history;
    } else {
...
export const changeStockOwnership = (history: StockHistory, i: number, change: number): StockHistory => {
    const init = history.slice(0, -1);
    const [h, a, b, z] = history[history.length - 1];
    return init.concat([[h.map((c, j) => j !== i ? c : [c[0], c[1], c[2], c[3], c[4], c[5], c[6] + change] as Company), a, b, z]]);
};
