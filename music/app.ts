...
console.log('hello');
type Guitar = [number, monzo.EquivScale][];
type Piano = [[number, string, monzo.Interval][], monzo.Interval, number, number];

let context = new AudioContext();

const draw = (guitar: Guitar, piano: Piano) => { // TODO: For piano, use a flexbox to space black keys evenly in groups
    let div = document.createElement('div');
    let fretMap: [monzo.Interval, HTMLDivElement][] = [];
    //add id
    div.id = 'guitar';
    ...
    for (let string of guitar) {
        ...
        let fretSequence = notes.map(note => note.inverse().ratio().interval.value).reduce((r: number[], e, i, a) => i ? r.concat(a[i - 1] - e) : r, []);
        i = 0;
    ...
    let oscillators: [monzo.Interval, OscillatorNode, GainNode][] = [];
