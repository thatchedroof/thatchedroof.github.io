...
let currentStocks = [[chosenCompanies, 1, stocks.mu, stocks.sigma]];
let money = 1000;
...
const update = () => {
        ...
        setTimeout(() => {
            let charts = chosenCompanies.map((c, j) => {
                return CandlestickChart(dataize(currentStocks.slice(-182).slice(0, i), 4, j, 182), {
                    ...
                    //xRange: [0, 182],
                    yLabel: "↑ Price ($)",
                    width: bottom.firstElementChild.firstElementChild.offsetWidth,
                    height: bottom.firstElementChild.firstElementChild.offsetHeight,
                });
            });
    ...
    setTimeout(() => {
        ...
        bottom.innerHTML = `<div class="cards">`.concat(...lastStock[0].map((c, j) => {
...
const changeStock = (i, inputChange) => {
    ...
    newMoney = money - change * currentStocks[currentStocks.length - 1][0][i][4];
    money = newMoney;
    currentStocks = stocks.changeStockOwnership(currentStocks, i, change);
...
// });
function dataize(history, chunkSize, company, fill = undefined) {
    let chunks = history.reduce((resultArray, item, index) => {
...
;
function portfolioDataize(history, chunkSize) {
    let chunks = history.reduce((resultArray, item, index) => {
        const chunkIndex = Math.floor(index / chunkSize);
        if (!resultArray[chunkIndex]) {
            resultArray[chunkIndex] = []; // start a new chunk
 ...
 */
function CandlestickChart(data, { date = (d) => d.date, // given d in data, returns the (temporal) x-value
...
xDomain = undefined, // array of x-values (defaults to every weekday)
xRange = undefined, // [left, right]
xPadding = 0.2, xTicks = undefined, // array of x-values to label (defaults to every other Monday)
...
colors = ["#4daf4a", "#999999", "#e41a1c"] // [up, no change, down]
    ...
    console.log(X);
    const weeks = (start, stop, stride) => d3.utcMonday.every(stride).range(new Date(start), +new Date(new Date(stop).getDate() + 1)).map(n => n.toISOString().split('T')[0]);
