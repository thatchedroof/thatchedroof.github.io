<script lang="ts">
    ...
    function buttonPress(text: string) {
        let acronyms = getAcronyms(text, disylls, cob).sort(
            (x, y) => y[1] - x[1],
        );
        ...
        for (let i = 0; i < acronyms.length; i++) {
            if (acroDiv.offsetHeight > 500) {
                ...
                (acroDiv.lastElementChild! as HTMLElement).style.visibility =
                    'hidden';
                break;
            }
            acroDiv.innerHTML += `
...
<svelte:head>
    ...
    <meta
        property="og:url"
        content="https://thatchedroof.github.io/auto-acronym/"
    />
...
<ul>
    ...
    <li class="button">
        <button type="button" on:click={importedButtonPress}>
            Generate acronyms!
        </button>
    </li>
</ul>
